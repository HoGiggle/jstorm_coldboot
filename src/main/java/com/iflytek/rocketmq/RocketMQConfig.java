...
public class RocketMQConfig implements Serializable {
    ...
    public static   RocketMQConfig getRocketMQConfig(Config config){
        //2.
        String topic = config.getString("topic");
        String consumerGroup = config.getString("consumer.group");
        String topicTag = config.getString("topic_tag");
        Integer pullBatchSize = config.getInt("prefetch.size");
        RocketMQConfig mqConfig = new RocketMQConfig(consumerGroup, topic, topicTag);
        String ip=config.getString("ip");
        mqConfig.setNamesrvAddr(ip);
        if (pullBatchSize != null && pullBatchSize > 0) {
            mqConfig.setPullBatchSize(pullBatchSize);
        }

        boolean ordered = config.getBoolean("ordered");
